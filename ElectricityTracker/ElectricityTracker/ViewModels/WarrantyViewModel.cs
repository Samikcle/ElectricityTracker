...
namespace ElectricityTracker.ViewModels
{
    public class WarrantyViewModel : BaseViewModel
    {
        ...
        async void OnItemSelected(ApplianceFmt item)
        {
            if (item == null)
                return;
            Routing.RegisterRoute("//WarrantyDetailPage", typeof(WarrantyDetailPage));
            // This will push the ItemDetailPage onto the navigation stack
            await Shell.Current.GoToAsync($"{nameof(WarrantyDetailPage)}?{nameof(WarrantyDetailViewModel.DeviceId)}={item.Id}");
        }
    }
}